...
highp vec2 zoffset(highp float zcoord, float dist)
{
    return vec2(zcoord + dist, zcoord) * 0.36;
}

void main()
{
    highp vec4 worldposition = pk_MATRIX_M * vec4(in_POSITION0, 1.0f);

    vs_TEXCOORD0.xz = zoffset(worldposition.z, pk_Distance);
